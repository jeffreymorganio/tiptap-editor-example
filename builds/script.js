(() => {
  ...
  var AddNodeMarkStep2 = class extends Step2 {
    constructor(pos, mark) {
      super();
      this.pos = pos;
      this.mark = mark;
  ...
  var RemoveNodeMarkStep2 = class extends Step2 {
    constructor(pos, mark) {
      super();
      this.pos = pos;
      this.mark = mark;
  ...
  var AttrStep2 = class extends Step2 {
    constructor(pos, attr, value) {
      super();
      this.pos = pos;
      this.attr = attr;
    ...
    invert(doc4) {
      return new AttrStep2(this.pos, this.attr, doc4.nodeAt(this.pos).attrs[this.attr]);
    }
    map(mapping) {
      let pos = mapping.mapResult(this.pos, 1);
  ...
  function makeRecover3(index, offset) {
    return index + offset * factor163;
  }
  ...
  var StepMap3 = class {
    constructor(ranges, inverted = false) {
      this.ranges = ranges;
      this.inverted = inverted;
      if (!ranges.length && StepMap3.empty)
        return StepMap3.empty;
    }
    recover(value) {
      let diff = 0, index = recoverIndex3(value);
      if (!this.inverted)
        for (let i = 0; i < index; i++)
          diff += this.ranges[i * 3 + 2] - this.ranges[i * 3 + 1];
      return this.ranges[index * 3] + diff + recoverOffset3(value);
    }
    mapResult(pos, assoc = 1) {
      return this._map(pos, assoc, false);
    }
    ...
    _map(pos, assoc, simple) {
      ...
      for (let i = 0; i < this.ranges.length; i += 3) {
        ...
        if (pos <= end) {
          ...
          let del2 = pos == start ? DEL_AFTER3 : pos == end ? DEL_BEFORE3 : DEL_ACROSS3;
          if (assoc < 0 ? pos != start : pos != end)
            del2 |= DEL_SIDE3;
          return new MapResult3(result, del2, recover);
        }
        diff += newSize - oldSize;
      }
    ...
    touches(pos, recover) {
      let diff = 0, index = recoverIndex3(recover);
      let oldIndex = this.inverted ? 2 : 1, newIndex = this.inverted ? 1 : 2;
      for (let i = 0; i < this.ranges.length; i += 3) {
        let start = this.ranges[i] - (this.inverted ? diff : 0);
        if (start > pos)
  ...
  var Step3 = class {
    ...
    static jsonID(id, stepClass) {
      if (id in stepsByID3)
        throw new RangeError("Duplicate use of step JSON ID " + id);
      stepsByID3[id] = stepClass;
      stepClass.prototype.jsonID = id;
      return stepClass;
    }
  };
  var StepResult3 = class {
    constructor(doc4, failed) {
      this.doc = doc4;
      this.failed = failed;
    }
    static ok(doc4) {
      return new StepResult3(doc4, null);
    }
    static fail(message) {
    ...
    static fromReplace(doc4, from2, to, slice2) {
      try {
        return StepResult3.ok(doc4.replace(from2, to, slice2));
      } catch (e) {
        if (e instanceof ReplaceError)
  ...
  function mapFragment3(fragment, f, parent) {
    let mapped = [];
    for (let i = 0; i < fragment.childCount; i++) {
      let child = fragment.child(i);
      if (child.content.size)
        child = child.copy(mapFragment3(child.content, f, child));
      if (child.isInline)
        child = f(child, parent, i);
      mapped.push(child);
    }
  ...
  var AddMarkStep3 = class extends Step3 {
    constructor(from2, to, mark) {
      super();
      this.from = from2;
      this.to = to;
    ...
    apply(doc4) {
      let oldSlice = doc4.slice(this.from, this.to), $from = doc4.resolve(this.from);
      let parent = $from.node($from.sharedDepth(this.to));
      let slice2 = new Slice(mapFragment3(oldSlice.content, (node, parent2) => {
        if (!node.isAtom || !parent2.type.allowsMarkType(this.mark.type))
          return node;
        return node.mark(this.mark.addToSet(node.marks));
      }, parent), oldSlice.openStart, oldSlice.openEnd);
      return StepResult3.fromReplace(doc4, this.from, this.to, slice2);
    }
    invert() {
    ...
    map(mapping) {
      ...
      if (from2.deleted && to.deleted || from2.pos >= to.pos)
        return null;
      return new AddMarkStep3(from2.pos, to.pos, this.mark);
    }
    merge(other) {
    ...
    static fromJSON(schema, json) {
      if (typeof json.from != "number" || typeof json.to != "number")
        throw new RangeError("Invalid input for AddMarkStep.fromJSON");
      return new AddMarkStep3(json.from, json.to, schema.markFromJSON(json.mark));
    }
  };
  Step3.jsonID("addMark", AddMarkStep3);
  var RemoveMarkStep3 = class extends Step3 {
    constructor(from2, to, mark) {
      super();
      this.from = from2;
      this.to = to;
    ...
    apply(doc4) {
      let oldSlice = doc4.slice(this.from, this.to);
      let slice2 = new Slice(mapFragment3(oldSlice.content, (node) => {
        return node.mark(this.mark.removeFromSet(node.marks));
      }, doc4), oldSlice.openStart, oldSlice.openEnd);
      return StepResult3.fromReplace(doc4, this.from, this.to, slice2);
    }
    ...
    map(mapping) {
      ...
      if (from2.deleted && to.deleted || from2.pos >= to.pos)
        return null;
      return new RemoveMarkStep3(from2.pos, to.pos, this.mark);
    }
    merge(other) {
    ...
    static fromJSON(schema, json) {
      if (typeof json.from != "number" || typeof json.to != "number")
        throw new RangeError("Invalid input for RemoveMarkStep.fromJSON");
      return new RemoveMarkStep3(json.from, json.to, schema.markFromJSON(json.mark));
    }
  };
  Step3.jsonID("removeMark", RemoveMarkStep3);
  var ReplaceStep3 = class extends Step3 {
    constructor(from2, to, slice2, structure = false) {
      super();
      this.from = from2;
      this.to = to;
    ...
    invert(doc4) {
      return new ReplaceStep3(this.from, this.from + this.slice.size, doc4.slice(this.from, this.to));
    }
    map(mapping) {
      let from2 = mapping.mapResult(this.from, 1), to = mapping.mapResult(this.to, -1);
      if (from2.deletedAcross && to.deletedAcross)
        return null;
      return new ReplaceStep3(from2.pos, Math.max(from2.pos, to.pos), this.slice);
    }
    merge(other) {
      ...
      if (this.from + this.slice.size == other.from && !this.slice.openEnd && !other.slice.openStart) {
        let slice2 = this.slice.size + other.slice.size == 0 ? Slice.empty : new Slice(this.slice.content.append(other.slice.content), this.slice.openStart, other.slice.openEnd);
        return new ReplaceStep3(this.from, this.to + (other.to - other.from), slice2, this.structure);
      } else if (other.to == this.from && !this.slice.openStart && !other.slice.openEnd) {
        let slice2 = this.slice.size + other.slice.size == 0 ? Slice.empty : new Slice(other.slice.content.append(this.slice.content), other.slice.openStart, this.slice.openEnd);
        return new ReplaceStep3(other.from, this.to, slice2, this.structure);
      } else {
        return null;
      }
    }
    ...
    static fromJSON(schema, json) {
      if (typeof json.from != "number" || typeof json.to != "number")
        throw new RangeError("Invalid input for ReplaceStep.fromJSON");
      return new ReplaceStep3(json.from, json.to, Slice.fromJSON(schema, json.slice), !!json.structure);
    }
  };
  Step3.jsonID("replace", ReplaceStep3);
  var ReplaceAroundStep3 = class extends Step3 {
    constructor(from2, to, gapFrom, gapTo, slice2, insert, structure = false) {
      super();
      this.from = from2;
      this.to = to;
    ...
    invert(doc4) {
      let gap = this.gapTo - this.gapFrom;
      return new ReplaceAroundStep3(this.from, this.from + this.slice.size + gap, this.from + this.insert, this.from + this.insert + gap, doc4.slice(this.from, this.to).removeBetween(this.gapFrom - this.from, this.gapTo - this.from), this.gapFrom - this.from, this.structure);
    }
    map(mapping) {
      let from2 = mapping.mapResult(this.from, 1), to = mapping.mapResult(this.to, -1);
      let gapFrom = mapping.map(this.gapFrom, -1), gapTo = mapping.map(this.gapTo, 1);
      if (from2.deletedAcross && to.deletedAcross || gapFrom < from2.pos || gapTo > to.pos)
        return null;
      return new ReplaceAroundStep3(from2.pos, to.pos, gapFrom, gapTo, this.slice, this.insert, this.structure);
    }
    toJSON() {
      let json = {
        stepType: "replaceAround",
    ...
    static fromJSON(schema, json) {
      if (typeof json.from != "number" || typeof json.to != "number" || typeof json.gapFrom != "number" || typeof json.gapTo != "number" || typeof json.insert != "number")
        throw new RangeError("Invalid input for ReplaceAroundStep.fromJSON");
      return new ReplaceAroundStep3(json.from, json.to, json.gapFrom, json.gapTo, Slice.fromJSON(schema, json.slice), json.insert, !!json.structure);
    }
  };
  Step3.jsonID("replaceAround", ReplaceAroundStep3);
  function contentBetween3(doc4, from2, to) {
    let $from = doc4.resolve(from2), dist = to - from2, depth = $from.depth;
    while (dist > 0 && depth > 0 && $from.indexAfter(depth) == $from.node(depth).childCount) {
      depth--;
      dist--;
  ...
  function canCut2(node, start, end) {
    return (start == 0 || node.canReplace(start, node.childCount)) && (end == node.childCount || node.canReplace(0, end));
  }
  function liftTarget2(range) {
    let parent = range.parent;
    let content = parent.content.cutByIndex(range.startIndex, range.endIndex);
    for (let depth = range.depth; ; --depth) {
      let node = range.$from.node(depth);
      ...
      if (depth < range.depth && node.canReplace(index, endIndex, content))
        return depth;
      if (depth == 0 || node.type.spec.isolating || !canCut2(node, index, endIndex))
        break;
    }
    return null;
  }
  function findWrapping2(range, nodeType, attrs = null, innerRange = range) {
    let around = findWrappingOutside2(range, nodeType);
    let inner = around && findWrappingInside2(innerRange, nodeType);
    if (!inner)
      return null;
    return around.map(withAttrs2).concat({ type: nodeType, attrs }).concat(inner.map(withAttrs2));
  }
  function withAttrs2(type) {
    return { type, attrs: null };
  }
  function findWrappingOutside2(range, type) {
    let { parent, startIndex, endIndex } = range;
    let around = parent.contentMatchAt(startIndex).findWrapping(type);
    if (!around)
      return null;
  ...
  function findWrappingInside2(range, type) {
    let { parent, startIndex, endIndex } = range;
    let inner = parent.child(startIndex);
    let inside = type.contentMatch.findWrapping(inner.type);
    if (!inside)
  ...
  function canSplit2(doc4, pos, depth = 1, typesAfter) {
    let $pos = doc4.resolve(pos), base2 = $pos.depth - depth;
    let innerType = typesAfter && typesAfter[typesAfter.length - 1] || $pos.parent;
    if (base2 < 0 || $pos.parent.type.spec.isolating || !$pos.parent.canReplace($pos.index(), $pos.parent.childCount) || !innerType.type.validContent($pos.parent.content.cutByIndex($pos.index(), $pos.parent.childCount)))
      return false;
  ...
  var clearNodes = () => ({ state, tr, dispatch }) => {
    ...
    ranges.forEach(({ $from, $to }) => {
      state.doc.nodesBetween($from.pos, $to.pos, (node, pos) => {
        ...
        const targetLiftDepth = liftTarget2(nodeRange);
  ...
  function selectionToInsertionEnd2(tr, startLen, bias) {
    ...
    if (!(step instanceof ReplaceStep3 || step instanceof ReplaceAroundStep3)) {
      return;
    }
  ...
  var splitBlock2 = ({ keepMarks = true } = {}) => ({ tr, state, dispatch, editor }) => {
    ...
    if (selection instanceof NodeSelection && selection.node.isBlock) {
      if (!$from.parentOffset || !canSplit2(doc4, $from.pos)) {
        return false;
      }
      if (dispatch) {
        if (keepMarks) {
    ...
    if (dispatch) {
      ...
      let can = canSplit2(tr.doc, tr.mapping.map($from.pos), 1, types);
      if (!types && !can && canSplit2(tr.doc, tr.mapping.map($from.pos), 1, deflt ? [{ type: deflt }] : void 0)) {
        can = true;
        types = deflt ? [{
          type: deflt,
          attrs: newAttributes
  ...
  var joinListBackwards = (tr, listType) => {
    ...
    const canJoinBackwards = list.node.type === (nodeBefore === null || nodeBefore === void 0 ? void 0 : nodeBefore.type) && canJoin2(tr.doc, list.pos);
    if (!canJoinBackwards) {
      return true;
    }
    tr.join(list.pos);
  ...
  var joinListForwards = (tr, listType) => {
    ...
    const canJoinForwards = list.node.type === (nodeAfter === null || nodeAfter === void 0 ? void 0 : nodeAfter.type) && canJoin2(tr.doc, after);
    if (!canJoinForwards) {
      return true;
    }
    tr.join(after);
...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  function parseFromClipboard2(view, text, html, plainText, $context) {
    let inCode = $context.parent.type.spec.code;
    let dom, slice2;
    if (!html && !text)
      return null;
  ...
  handlers2.copy = editHandlers2.cut = (view, _event) => {
    ...
    let slice2 = sel.content(), { dom, text } = serializeForClipboard2(view, slice2);
    if (data) {
      event.preventDefault();
      data.clearData();
      data.setData("text/html", dom.innerHTML);
  ...
  function doPaste2(view, text, html, event) {
    let slice2 = parseFromClipboard2(view, text, html, view.input.shiftKey, view.state.selection.$from);
    if (view.someProp("handlePaste", (f) => f(view, event, slice2 || Slice.empty)))
      return true;
    if (!slice2)
      return false;
  ...
  handlers2.dragstart = (view, _event) => {
    ...
    let slice2 = view.state.selection.content(), { dom, text } = serializeForClipboard2(view, slice2);
    event.dataTransfer.clearData();
  ...
  // js/tiptap_editor_controller.js
  var TipTapEditorController = class {
    ...
    addButtonListeners() {
      this.addButtonListener("heading-1", (chain) => {
        return chain.toggleHeading({ level: 1 });
      });
      this.addButtonListener("heading-2", (chain) => {
        return chain.toggleHeading({ level: 2 });
      });
      this.addButtonListener("heading-3", (chain) => {
        return chain.toggleHeading({ level: 3 });
      });
      this.addButtonListener("bold", (chain) => {
        return chain.toggleBold();
      });
      this.addButtonListener("italic", (chain) => {
        return chain.toggleItalic();
      });
      this.addButtonListener("strike", (chain) => {
        return chain.toggleStrike();
      });
      this.addButtonListener("bulletList", (chain) => {
        return chain.toggleBulletList();
      });
      this.addButtonListener("orderedList", (chain) => {
        return chain.toggleOrderedList();
      });
      this.addButtonListener("undo", (chain) => {
        return chain.undo();
      });
      this.addButtonListener("redo", (chain) => {
        return chain.redo();
      });
    }
    addButtonListener(dataAttribute, command2, isToggleable = false) {
      ...
      buttonElements.forEach((buttonElement) => {
        ...
        buttonElement.addEventListener("click", (event) => {
          command2(this.editor.chain().focus()).run();
          this.updateButtons();
        });
      });
    }
    ...
    updateHeadingButtons() {
      [1, 2, 3].forEach((level) => {
        ...
        const buttonOn = this.editor.isActive("heading", { level });
  ...
  // js/script.js
  var tiptapEditorController = new TipTapEditorController("editor", "Hello World from TipTap!");
})();
